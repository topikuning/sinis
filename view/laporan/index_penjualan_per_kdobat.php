...
$(document).ready(function() {
	...
	$('#cetkar').click(function(){
		...
		$.ajax({
			type: 'POST',
			url: "act_index_penjualan_per_kdobat.php",
			data: $(this).serialize(),
			success: function(data) {
				//$('#result').html(data);
				var win = window.open('act_index_penjualan_per_kdobat.php?cetak=print&ob1='+ob1+"&ob2="+ob2+"&tgla="+tgla+"&tglb="+tglb,'CetakDong','height=500,width=1200,resizable=1,scrollbars=0, menubar=0');
			}
		})
	});
})
...
<TABLE border='0' cellpadding='3' cellspacing='3'  width='800' id='print'>
<TR>
	<TD>
	
            <form id="myForm" name="myForm" method="post" action="act_index_penjualan_per_kdobat.php">
                <table align="left" style=' font-family: verdana; font-size: 11px;' width='100%' border='0' cellpadding='3' cellspacing='1'>
                    <tr>
                        <td width='1%'  align='center' bgcolor='#00CC00' colspan='4'><B>Laporan Penjualan Per Kode Obat</B></td>
                    </tr>
                    <tr bgcolor='#D1FDCC'>
                        <td  width='200'>
                            <label>Obat:</label>
                        ...
                        <td>
                            <select id="obat1" name="obat1">
                                <option value="">[Pilih Obat]</option>
                                <?
                                $query = "SELECT id_obat, nama_obat FROM rm_obat order by id_obat";

                                $result = mysql_query($query);
                                while ($dt = mysql_fetch_array($result, MYSQL_ASSOC)) {
                                    echo "<option value=\"$dt[id_obat]\">$dt[nama_obat]</option>";
                                }
                                ?>
                            </select>
                        </td>
            ...
            <br>*) Dapat digunakan pencarian satu obat saja.
	</TD>
</TR>
</TABLE>
