...
<TR>
	...
	<form id="myForm" name="myForm" method="post" action="act_index_informasi.php">
	<table align="left" style=' font-family: verdana; font-size: 11px;' width='100%' border='0' cellpadding='3' cellspacing='1'>
	<tr>
		<td width='1%'  align='center' bgcolor='#00CC00' colspan='4'><B>Search Informasi Pasien</B></td>
	</tr>
	<!-- <tr bgcolor='#D1FDCC'>
		<td  width='200'>
		...
		<td colspan='3'>
			...
			<?
			$query = "SELECT id_penyakit,nama_penyakit FROM rm_penyakit order by id_penyakit";

			$result = mysql_query($query);
			while ($dt = mysql_fetch_array($result, MYSQL_ASSOC)) {
			echo "<option value=\"$dt[id_penyakit]\"> $dt[id_penyakit] - $dt[nama_penyakit]</option>";
			}
			?>
			</select>
		</td>
	...
	<tr bgcolor='#D1FDCC'>
		<td></td>
		<td colspan='3'>Note : Bila ada data yang sama, maka data teratas ruang yang ditempati</td>
	</tr>
	<tr>
