...
$(document).ready(function() {
	...
	$('#cetkar').click(function(){
		...
		$.ajax({
			type: 'POST',
			url: "act_index_tindakan_rawat_jalan.php",
			data: $(this).serialize(),
			success: function(data) {
				//$('#result').html(data);
				var win = window.open('act_index_tindakan_rawat_jalan.php?cetak=print&idt='+idt+"&tgla="+tgla+"&tglb="+tglb,'CetakDong','height=500,width=1200,resizable=1,scrollbars=0, menubar=0');
			}
		})
	});
})
...
<TR>
	...
	<form id="myForm" name="myForm" method="post" action="act_index_tindakan_rawat_jalan.php">
	<table align="left" style=' font-family: verdana; font-size: 11px;' width='100%' border='0' cellpadding='3' cellspacing='1'>
	<tr>
		<td width='1%'  align='center' bgcolor='#00CC00' colspan='4'><B>Laporan Tindakan Rawat Jalan</B></td>
	</tr>
	<tr bgcolor='#D1FDCC'>
		<td  width='200'>
